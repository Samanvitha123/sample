	...
	public String RegisterEvent(EventBean evnt)
	{
		 ...
		 Connection con = null;
	        ...
	        try
	        {
	            ...
	            int k = statement.executeUpdate("insert into event(event_name,event_descr,dateofevent,addrs,phone,email,rules) values('"+event_name+"','"+event_descr+"','"+dateofevent+"','"+addrs+"','"+phone+"','"+email+"','"+rules+"')");//("select userName,password from users"); //the table name is users and userName,password are columns. Fetching all the records and storing in a resultSet.
