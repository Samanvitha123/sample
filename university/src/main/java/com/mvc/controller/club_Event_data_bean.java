 ...
 * @author DELL
 */
public class club_Event_data_bean extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet club_Event_data_bean</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet club_Event_data_bean at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        PrintWriter out = response.getWriter();
		 ...
		 evnt.setEmail(email);
		 evnt.setRules(rules);
		      club_event_dao loginDao = new club_event_dao(); //creating object for LoginDao. This class contains main logic of the application.
	        ...
	        if(userValidate.equals("SUCCESS")) //If function returns success string then user will be rooted to Home page
	         ...
	         //    HttpSession session=request.getSession();  
	           //  session.setAttribute("uname",username);
	        	out.println("<script> alert(\"club event Inserted succefully!\");</script>");
