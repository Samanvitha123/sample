...
String rules=request.getParameter("rules");
        ...
        bean.setId(id);
        bean.setEvent_name(name);
        bean.setEvent_descr(descr);
        bean.setAddrs(adrs);
        bean.setDateofevent(doe);
        bean.setEmail(email);
        bean.setPhone(phone);
        bean.setRules(rules);
         Updateevent loginDao = new Updateevent(); //creating object for LoginDao. This class contains main logic of the application.
 
        String userValidate = loginDao.updateevent(bean); //Calling authenticateUser function
      ...
      //       request.getRequestDispatcher("/AdminHome.jsp").forward(request, response);//RequestDispatcher is used to send the control to the invoked page.
        	out.println("<script> alert(\"event Updated succefully!\");</script>");
...
String rules=request.getParameter("rules");
      ...
      //       request.getRequestDispatcher("/AdminHome.jsp").forward(request, response);//RequestDispatcher is used to send the control to the invoked page.
        	out.println("<script> alert(\"event Updated succefully!\");</script>");
	        	out.println("<script> location.replace('http://localhost:8085/university//AdminHome.jsp', '_blank')</script>");
